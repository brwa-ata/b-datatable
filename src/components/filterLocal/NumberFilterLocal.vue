    ...
    <div class="nfl__row">
      ...
      >
        ...
        >
          ...
          >
            <button
              class="nfl__dropdown-item"
              type="button"
              :class="{ 'nfl__dropdown-item--active': option.value === symbol }"
            >
              {{ option.title }}
            </button>
          </li>
      ...
      <input
        v-model="searchValue"
        :id="`number-filter-local-input__${props.columnKey}`"
        placeholder="Search..."
        type="text"
        autocomplete="off"
        class="nfl__input"
      ...
      <button
        class="nfl__clear-btn"
        type="button"
        :class="{ 'nfl__clear-btn--hidden': !filterApplied }"
...
import type { LocalNumberSymbol } from './localFilter'
...
const emit = defineEmits<{
  'value-changed': [payload: { key: string; symbol: LocalNumberSymbol; value: string }]
}>()

const dropdownWrapper = ref<HTMLDivElement | null>(null)
const showDropdown = ref(false)
const symbol = ref<LocalNumberSymbol>('=')
const searchValue = ref('')
const filterApplied = ref(false)
const numberFilterOptions: Array<{ title: string; value: LocalNumberSymbol }> = [
  { title: 'Equal (=)', value: '=' },
...
/** Keep only a valid (optionally negative / decimal) number out of the input. */
function onlyNumber(val: string): string {
  const match = val.match(/-?\d+(\.\d+)?/g)
  return match ? match.join('').replaceAll(',', '') : ''
}

function symbolChanged(val: LocalNumberSymbol) {
  symbol.value = val
  showDropdown.value = false
  if (searchValue.value.length) {
    applyFilter()
  }
  document.getElementById(`number-filter-local-input__${props.columnKey}`)?.focus()
}
...
function applyFilter() {
  ...
  filterApplied.value = searchValue.value.length > 0
  emitChange()
}
</script>
...
.nfl {
  ...
  &__clear-btn {
    ...
    &:not(.nfl__clear-btn--hidden):hover {
      opacity: 0.7;
    }
  }
}
